...
class KFIndexBarLineModelTests: XCTestCase {
    ...
    func testOuterItemPositions0() {
        ...
        lineModel.setOuterItemSizes([4.0, 1.0, 3.0])
    ...
    func testInnerItemPositions0() {
        ...
        lineModel.setOuterItemSizes([4.0, 1.0, 3.0])
        lineModel.setInnerItemSizes([2.0, 2.0, 1.0, 2.0], openBelow: 0)
    ...
    func testInnerItemPositionsHalfway() {
        var lineModel = KFIndexBar.LineModel(length: 10.0)
        lineModel.setOuterItemSizes([4.0, 1.0, 3.0])
        lineModel.setInnerItemSizes([2.0, 2.0, 1.0, 2.0], openBelow: 0)
        let inner = lineModel.calculateInnerPositions(forZoomExtent: 0.5)
        XCTAssertEqual(inner, [2.75, 4.25, 5.5, 6.75])
    }
}
